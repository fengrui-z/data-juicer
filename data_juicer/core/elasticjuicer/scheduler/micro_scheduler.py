...
class BatchSizeController:
    ...
    def report_oom(self, batch_size: int, memory_mb: float):
        ...
        self.max_batch_size = max(self.min_batch_size, batch_size - 1)
        self.pid.output_limits = (self.min_batch_size, self.max_batch_size)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_batch_size": (
                np.mean([h["new_batch"] for h in self.batch_size_history]) if self.batch_size_history else 0
            ),
        }
