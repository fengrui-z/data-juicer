...
class Captain:
    ...
    def __init__(self, config: CaptainConfig, tower_callback: Optional[Callable[[StageMetrics], None]] = None):
        """
    ...
    def process_batch(self, operator_func: Callable, sample_batch: Optional[List] = None) -> Optional[List]:
        """
    ...
    def _handle_oom(self, batch_size: int, snapshot: Optional[ResourceSnapshot]):
        ...
        # Emergency backoff
        if self.micro_scheduler:
            self.micro_scheduler.report_oom(
                batch_size,
                snapshot.memory_mb if snapshot is not None else 0,
            )
    ...
    def _check_quota_compliance(self):
        ...
        if current_memory_mb > self.quota.memory_quota_mb:
            if self.micro_scheduler:
                ...
                new_batch_size = max(1, int(self.micro_scheduler.controller.current_batch_size * reduction_ratio))
...
class CaptainPool:
    ...
    def get_all_stats(self) -> dict[str, dict]:
        ...
        return {name: captain.get_stats() for name, captain in self.captains.items()}
